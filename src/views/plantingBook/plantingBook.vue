  ...
  <div class="main">
    <div class="outLine">
      ...
      <div class="whitebg" v-if="ty === 'zh'">
        ...
        <p>题材：</p>
        <input type="text" v-model="artwork">
        <p>其他要求(选填):</p>
        ...
        <div class="copybut needsclick" @click="issend ? sent() : ensure()">{{ issend ? '已发送' : '确认无误，发送至邮件预订。' }}
        </div>
        <div class="switch" @click="switchLau()">ENGLISH</div>
      </div>

      <div class="whitebg" v-if="ty === 'en'">
        <span>The cost of illustration design is: 20000 RMB/piece.</span>
        <span>Size :80x100cm<br />Material:Chinese art paper </span>
        <span>*<br /> Illustration and tattoo carrier is different, may not be directly used for tattoo production,
          please do not ask for online manuscript or electronic version.</span>

        <span>It takes six to nine months to create, you pay the full amount to start,<br />Painting reservations do not
          accept revisions and refunds, please be sure to consider them fully.</span>
        ...
        <p>Email:</p>
        <input type="text" v-model="email">
        <div class="copybut needsclick" @click="issend ? sent() : ensure()">{{ issend ? 'has been sent' : 'Confirm and send to email booking.' }}</div>
...
export default {
  ...
  methods: {
    ...
    sent() {
      this.$message.error(this.ty === 'zh' ? '已发送' : 'have been sent')
    },
    copy() {
    ...
    toTop() {
      ...
      if (!this.social) {
        this.$message(this.ty === 'zh' ? '请填写微信' : 'Please fill in the insgram')
        return
      }
      if (!this.email) {
    ...
    sendemil() {
      this.sending = true
      emailjs.send('service_tzq4tgc', 'template_y81n5pj', { subject: '插画预定', artwork: this.artwork, tips: this.tips, social: this.social,email:this.email }, 'efGrq8haGJ6-tGyBS').then((res) => {
        this.show = false
        ...
        this.sending = false
        console.log(errpr)
        this.$message.error(this.ty === 'zh' ? '发送失败,请稍后再试' : 'Failed to send, please try again later')//anttop@qq.com
      })
    },
  }
}
